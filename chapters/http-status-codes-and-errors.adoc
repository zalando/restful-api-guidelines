...
* A batch or bulk request *may* return {4xx}/{5xx} status codes, if the
  failure cannot be related to individual items of the batch or bulk request,
  e.g. in case of a general overload situation.

* A batch or bulk response with status code {207} *always* returns as payload
  a multi-status object containing sufficient status and/or monitoring
