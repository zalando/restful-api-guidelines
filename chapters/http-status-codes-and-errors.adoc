...
The before rules apply _even in the case_ that processing of all individual
parts _fail_ or each part is executed _asynchronously_! They are intended
to allow clients to act on batch and bulk responses in a consistent way by
inspecting the individual results. This rules are intended to prevent
misinterpretation base on global response codes, i.e. we explicitly reject
...
/event-types/{name}/events`] as short cut without inspecting the result.

A proper bulk or batch response may look as follows:

[source,yaml]
----
Response:
  description: standard batch response object.
  type: object
  properties:
    items:
      type: array
