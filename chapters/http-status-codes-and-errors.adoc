...
[.indent]
Resource only supports generating content that is not listed in the {Accept}
...
[.indent]
The client did not provide a supported request body. Only applicable to methods
with a request body.


[.do-not-use]
[[status-code-417]]
==== 417 Locked {rfc-status-417} {do-not-use} {ALL}
[.indent]
...
update problem" is avoided (see <<181>>).
