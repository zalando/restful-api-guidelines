...
You may additionally return the created resource and/or
the `Location` response header (see details in <<standard-headers>>).
|{POST}, {PUT}
...
|[[status-code-204]]{204}|
Like {200}, but no response payload is returned. 
|{PUT}, {PATCH}, {DELETE}
...
Hint, you should not return {409}, but {200} in case of successful robust creation of resources 
