...
* `date` should be used for properties where no exact point in time is not required and day time-range is sufficient,
for instance, document dates, birthdays, ETAs (estimated time of arrival).
Without further context, `date` without time implies the time period from midnight to midnight in the local time zone.
