...
| `string` | `tz-id` | Time-zone id as defined by the https://en.wikipedia.org/wiki/List_of_tz_database_time_zones[IANA time zone database] standard (including daylight saving time definitions). |`"Europa/Berlin"`
...
`z` which are also accepted in {RFC-3339}[RFC 3339]. 
...
represented as strings in the format defined by {ISO-8601}[ISO 8601]
