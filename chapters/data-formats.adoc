...
[#167]
== {MUST} use JSON as standard format for the payload
...
[#168]
== {MAY} use non JSON media types for binary or other specific format data

Non JSON media types may be used in following cases:
...
* Transferring binary data or data whose structure is not relevant. This is
  ...
  is. A typical example use case is downloading images in formats JPG, PNG, GIF.
* Alternative specific data formats (e.g. PDF, DOC, XML) 
  may be supported additionally to JSON format via content negotiation.
...
define several data formats (like `date`, `time`, `email`, `url` and others) using 
standards from ISO and IETF. The following table lists the data formats and some additional 
formats that are particularly useful in an e-commerce environment. 
You should use these standard data formats, whenever applicable. 
