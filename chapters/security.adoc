...
one of the security schemes defined in the
...
security scheme -- it is based on {RFC-6750}[RFC 6750] defining the standard header 
...
In other, more specific situations e.g. with customer and partner facing APIs we 
...
section for details on how to define correctly.

*Note:* Do not use OpenAPI OAuth security scheme type and flows (e.g. `implicit`) 
