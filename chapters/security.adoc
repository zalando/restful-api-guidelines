...
////
//Prepared change for functional permission names:
Permission names in APIs must conform to the following pattern:
...
Permission names are recommended to conform to the functional naming schema
(see <<223>> for details and `<functional-name>` definition). The following
alternative pattern is *only* allowed for <<223, internal>> permissions:
