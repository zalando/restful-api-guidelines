...
to each functional component. It is build of domain name of the functional group
the component is belonging to and a unique a short identifier for functional
component itself:
...
The list of domains is curated by team architecture to foster consistent
grouping of components within the company application landscape. The
uniqueness of the `functional-name` is ensured by
https://github.bus.zalan.do/team-architecture/functional-component-registry[registering
(internal link)] all functional component.
...
[#224]
== {MUST} Use Functional Naming for Hostnames
...
The following application specific hostname schema is allowed for <<223,
internal>> APIs only:
