...
Hostnames in APIs must conform to the following pattern:
...
Hostnames are recommended to conform to the functional naming schema (see
<<223>> for details and `<functional-name>` definition). The following
application specific hostname pattern is *only* allowed for <<223,
internal>> APIs:
