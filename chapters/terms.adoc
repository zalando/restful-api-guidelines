...
The API Specification, of course, might differ from the API provided by a concrete service, because e.g. the service implementation is incomplete, not up-to-date or faulty. The API Specification that is supposed to be implemented by the service <<192, must / should be published with the service deployment>>. 
