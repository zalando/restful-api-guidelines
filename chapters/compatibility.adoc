...
* {TODO} Unknown input fields in payload or URL should not be ignored; servers should
...
[#114]
== {TODO} {MUST} use media type versioning
...
`application/x.nupano.app+json;version=2`. For incompatible changes, a new
...
[#110]
== {TODO} {MUST} always return JSON objects as top-level data structures
...
[#112]
== {TODO} {SHOULD} use open-ended list of values (`x-extensible-enum`) for enumeration types
