...
*Hint:* Note, there is a difference between incompatible and braking changes:
Incompatible changes are changes that are not covered by the compatibility 
rules below; they potentially break API consumers. 
Braking changes are changes that really brake running API consumers when deployed into operation.
Typically, incompatible changes are braking changes when deployed into (uncontrolled) operation environment. 
However, in specific controlled situations with no API consumers using the affected API aspect, 
incompatible changes are not braking changes (see also <<deprecation, Deprecation>> guidelines). 
...
API identifier together with the <<116,semantic API version>> reflect the
