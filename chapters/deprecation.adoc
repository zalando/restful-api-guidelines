...
The {Deprecation} header can either be set to `true` or carry a deprecation
time stamp, at which a replacement will become/became available and further
on-boarding is discouraged. The optional {Sunset} time stamp carries the
information when consumers latest have to switch to the replacement solution,
and should always offer an eligible time interval for switching.
...
responses to get information about future sunset of API features (see <<189>>).
