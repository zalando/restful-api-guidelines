...
generate a UUID value. However, due to "at-least-once" delivery guarantees
that most message brokers provide, event producers need to ensure that
subsequent attempts to publish an event do use the same identifier as the
initial (possibly failed) attempt.
