...
* Data Change Event: a category used to inform about changes to data
entities, and used for data replication based data integration.
...
You _must_ use data change events to signal changes of stored entity instances 
and facilitate e.g. change data capture (CDC), i.e. keeping datastore replicas 
in sync with minimal delays sourced from events.  Data change events are 
crucial e.g. for our data analytics integration architecture: They facilitate 
the logical replication of the microservice application datastores into 
transactional datasets as input for data analytics and AI. 

* Use the Data Change Event category when publishing events that represent 
created, updated, or deleted data.
* Change events must provide the identifier of the changed entity instance 
to allow aggregation of all related events for the entity.
* Change events <<203>>.
...
All data change events _must_ be provided with the information about the business 
transaction ordering of the events. In the event type definition, the `ordering_key_fields` 
property must be used to indicate which field(s) contains the ordering key. 
Additionally, the `ordering_instance_ids` property should be used to define 
which field represents the data instance identifier and scope in which 
`ordering_key_fields` provides a strict order. (If undefined, the ordering is 
assumed to be provided in scope of all events.)

The ordering information defines the transactional data change order managed 
by the application, and is used e.g. for analytics change data capture, i.e. 
keeping transactional data in sync as source for analytics. The ordering 
information is business knowledge located in the application's publisher and 
typically is implemented in scope of event partitions or event entity instances 
(but may also be implemented in scope of the event type for all events, if 
scaling requirements are low). The order information differs from Nakadi 
provided `occured_at` or `partition_offset` metadata, since application's event 
creation may not reflect the transactional order and Nakadi is a distributed 
concurrent system without ordered atomic event publication. 

The business ordering information can be provided – among other ways – by maintaining...

 * a strictly monotonically increasing entity instance version (e.g. as created 
   by a database) to allow for partial ordering of all events for an entity, or
 * a strictly monotonically increasing sequence counter (per partition or events type).

Hint: System timestamps are not necessarily a good choice, since usually there 
is no exact clock synchronization in distributed systems, and two
...
When using timestamps, the producer *must* make sure that they
...
ordering information needs to be provided.


[#242]
== {SHOULD} provide explicit event ordering for general events

While the order information is mandatory for data change events, it _should_ be also 
provided for general events (fka business events). Providing explicitly the order of 
business occurrence supports event consumers e.g. in error situations to 
reconstruct event streams or replay events from a position within the stream. 
For further details, please check <<203>>.
