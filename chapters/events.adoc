...
[#196]
== {MUST} Ensure Event schema conforms to Open API's Schema Object

To align event definitions with API resource definitions, we use
Schema Object from Open API to define events. This is particularly
useful for events that represent data changes about resources.

Schema Object is an "extended subset" of
http://json-schema.org/[JSON Schema Draft 4]. For your convenience,
we highlight some important differences below; please refer to the
https://github.com/OAI/OpenAPI-Specification/blob/master/versions/2.0.md#schemaObject[Schema Object specification] for details.

Schema Object _removes_ some JSON Schema keywords; the following properties
*must not* be used in event schemas:
...
Schema Object _redefines_ some keywords:
...
Schema Object _extends_ JSON Schema with some keywords:
...
See <<200>> for more guidance on how to use the category.
...
domain and adhere to its natural lifecycle (see also <<139>>).
...
Events that are designed for <<210,idempotent>> out-of-order
