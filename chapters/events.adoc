  ...
  properties:
    name:
      description: |
        ...
        `<functional-domain>.[<functional-component>.]<event>` that makes it
...
[#213]
== {MUST} Use functional naming schema for event names
...
[source,bnf]
----
<event-name>            ::= <functional-event-name> | <organizational-event-name>

<functional-event-name> ::= <functional-domain>.[<functional-component>].<event>
...
-- legacy format for internal event names only:
<organizational-event-name> ::= <organization-unit>.<application>.<event>
<organizational-unit>   ::= [a-z][a-z0-9-]* -- organizational unit, e.g. team identifier
<application>           ::= [a-z][a-z0-9-]* -- application identifier

<event>                 ::= [a-z][a-z0-9-]* -- free event identifier (functional name)
----

**Note:** The `functional-domain` and `functional-component` must correspond to
the functional name of the owning application. The organizational schema is
only allow for internal event names (see <<223>>).
