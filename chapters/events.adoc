...
in fact distinctly identify events. A straightforward way to create a
unique identifier for an event is to generate a UUID value. However, due
to "at-least-once" delivery guarantees that most message brokers provide,
event producers need to ensure that subsequent attempts to publish an
event do use the same identifier as the initial (possibly failed) attempt.

Considerations for event consumers are further detailed in <<212>> and
<<214>>. 
