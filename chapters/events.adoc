  ...
  properties:
    ...
    occurred_at:
      description: |
         ...
         The timestamp may differ from when the actual business event happened, e.g. 'packet delivered to the customer'. 
         In these situations we expect that the information is passed via an additional event type specific attribute e.g. 'delivered_at'. 
         (Hint: `occurred_at` timestamp also typically differs from when the event is posted by the producer and received by Nakadi via the API.) 
      type: string
      format: date-time
      example: '1996-12-19T16:39:57-08:00'
    received_at:
