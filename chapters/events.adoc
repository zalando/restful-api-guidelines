...
EventType:
  ...
  properties:
    name:
      ...
      example: transactions.order.order-cancelled, customer.personal-data.email-changed
    owning_application:
      description: |
...
[#213]
== {MUST-SHOULD} Use functional naming schema for event names

Event names in APIs are recommended to conform to the functional naming (see 
...
event-identifier     ::= [a-z][a-z0-9-]* ; free event identifier (functional name)
----

The `functional-domain` and `functional-component` must correspond to the
functional name of the owning application.

*Note:* If an entity of a data change events is also expose as resource via a
RESTful API, the event name should be the same as the resource name in the API.

If the functional name schema is not applied, the uniqueness and consistency
of the event name must be preserved by substituting the `functional-domain`
against the team name and the `functional-component` against the application
identifier.
