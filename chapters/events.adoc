...
*Note:* The `occured_at` and `partition_offset` metadata of Nakadi typically is 
different from the business event ordering, since (i) Nakadi is a distributed concurrent 
system without atomic, ordered event creation and (ii) the application's implementation
...
replication (and reconstruction) of the application datastores into 
transactional datasets used as input for data analytics and AI. 
...
to keep transactional data replicas in sync as source for data analytics. 
