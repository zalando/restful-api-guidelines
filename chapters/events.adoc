...
Pon’s architecture centers around decoupled microservices and in
...
https://github.com/cloudevents/spec/blob/v1.0/spec.md[CloudEvents], which acts as a
...
In Pon's architecture, events are registered using a structure
...
EventType:
  ...
  properties:
    ...
    type: 
      description: |
        Unique name or action description of the event. Most of the time its a string divided by '.' to specify the type of object and its action.
      type: string
      example: |
        com.github.pull.create
        exactonline.order.update
        com.swapfiets.order.create
        com.gazelle.delivery.update
    source:
      description: |
        A url or location where the event is coming from
      type: string
      example: |
        https://github.com/pondigitalsolutions/spec/pull
        https://api.products.gazelle.com/v2/product
    subject:
      description: |
        This describes the subject of the event in the context of the event producer (identified by the 'source'). 
        Usually a Uniqueidentifier or a unique value to identify the object mentioned in the context.
      type: string
      example: |
        54654348
        product-1234
    ...
    id:
      description: |
        This is the uniqueidentifier of the event message. 
      type: string
      example: |
        1234-1324-1234-1234
        A1342-376-97245-0.1
    time: 
      description: |
        OPTIONAL. This is the DateTime formatted string representing the created date of the context. Can be used to compare events which have the same subject involved.
      type: string
      format: date-time
      example: |
    ...
    datacontenttype:
      description: |
        This defines the Content-Type of the data object in the context.
      type: string
      example: |
        text/xml
        application/json
    data:
      description: |
        This is the custom section of the event in the context. Here you can add a custom object to the context in the Content-Type defined by 'datacontenttype'.
      type: string
      example: |
        <xml type="xmlns" />
----
...
example the https://nakadi.io/[Nakadi] API's event category registration also allows the
...
* The https://nakadi.io/[Nakadi] broker still refers to the General Category as the Business
...
example brokers such as https://nakadi.io/[Nakadi], can validate and enrich events with
