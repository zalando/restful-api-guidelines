...
Property names are restricted to ASCII strings. The first character must be
a lowercase letter, and subsequent characters can be a letter (lowercase or
uppercase), or a number.

Rationale: No established industry standard exists, but it’s essential to
establish a consistent look and feel such that JSON looks as if it came from
the same hand.
