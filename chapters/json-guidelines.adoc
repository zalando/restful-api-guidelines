...
boolean properties which otherwise would have very similar or even identical names:

- `created_at` and `modified_at` rather than `created` and `modified`
  The later ones were used in an earlier version of the guidelines and are therefore still accepted
  for APIs that predate this rule.
- `occurred_at`, rather than `occurred`
