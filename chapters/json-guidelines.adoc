...
If a property is defined to be `non-required` and `nullable` (see
<<required-nullable-row-2, 2nd row of below Table>>), this rule demands 
...
While API designers and implementers may be tempted to assign both cases a
different semantics, we explicitly decide **against** this options, because we
think that any gain in expressiveness is far outweighed by the risk of clients
not understanding and implementing the subtle differences correctly. Moreover,
many major libraries and frameworks as well as some strong-typed languages
simply do not support different semantics.
