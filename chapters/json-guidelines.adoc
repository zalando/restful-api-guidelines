...
[#125]
[#240]
== {SHOULD} declare enum values using UPPER_SNAKE_CASE string format

Enumerations should be represented as `string` typed OpenAPI component schemas. 
...
**Note:** This does not apply where the actual exact values are coming from some 
outside source, e.g. for language codes from {ISO-639-1}[ISO 639-1], or when 
declaring possible values for a #137[`sort` parameter].
