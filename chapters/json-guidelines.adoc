...
the same resource type. The usualy differences between read and write models
...
ignored or rejected by a server. Both is inline with the idea behind <<#109,
the compatibility guidance>>, however, we suggest to document rejection to
prevent client side surprises.
