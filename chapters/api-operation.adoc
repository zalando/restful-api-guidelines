...
An API is published by copying its **self-contained OpenAPI specification**
as single **YAML file** into the reserved **/zalando-apis** directory of the
**deployment artifact** used to deploy the provisioning service
(https://docs.google.com/a/zalando.de/document/d/1WBpRHp1DAtz7Pfjt0QwPuZNR1e70APRqUlCCby3JiOM[see
internal narrative for more details]). We prefer this deployment artifact-based
method over the past (now legacy) `.well-known/schema-discovery` service
endpoint-based publishing process, that we only support for backward
