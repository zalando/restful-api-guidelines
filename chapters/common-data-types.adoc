...
[[simple-hypertext-control-fields]]
=== Simple Hypertext Control and Pagination Fields

To foster a consistent look and feel of simple hypertext controls responses
for paginating over collection values should follow a common pattern using the
following field semantics:

* [[first]]{first}: the link or cursor in an object or pagination response
  pointing to the first object or page.
* [[prev]]{prev}: the link or cursor in an object or pagination response
  pointing to the previous object or page.
* [[self]]{self}: the link or cursor in an object or pagination response
  pointing to the same object or the current page.
* [[next]]{next}: the link or cursor in an object or pagination response
  pointing to the next object or page.
* [[last]]{last}: the link or cursor in an object or pagination response
  pointing to the last page.

To distinguish between pagination links and cursors the cursor set should be
wrapped in a {cursors} object as follows:

* [[cursors]]{cursors}: pagination object containing the cursors for the
  {first}, {prev}, {self}, {next}, and {last} page.
* [[cursor]]{cursor}: the cursor used in a {GET-with-body} search request
  pointing to the requested page.
* [[items]]{items}: array of resources, holding all the items of the current
  page (can be replaced by resource name).

For a standard response page this may look as follows:
...
Normally, only cursors or simple hypertext controls are used. The response page
may contain additional metadata about the collection or the current page, if
necessary.
