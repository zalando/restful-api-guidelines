...
[#173]
== {STATUS-RFP} {MUST} use the common money object
...
[#174]
== {STATUS-RFP} {MUST} use common field names and semantics
...
* [[xyz_id]]{xyz_id}: an attribute within one object holding the identifier of
  ...
  *Exception*: We use `customer_number` instead of `customer_id` for customer facing 
  identification of customers due to legacy reasons.
...
allows API client developers to build up a common understanding of Pon's
