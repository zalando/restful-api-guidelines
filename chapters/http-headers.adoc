...
components:
  ...
  - Idempotency-Key:
      description: |
        ...
        recommended to use a UUID version 4 (random) or any other random
...
and use the dash (`-`) as separator (dash-case). 
...
This include, for instance, non pass-through headers `X-Zalando-Customer`, `X-Zalando-Client-ID`, `X-Zalando-Request-Host`, 
