...
referenced by their identifier and name in the path segments as follows:
...
**Exception:** In some situations the id information is not passed 
as a path segment but via context information, e.g. the authorization token. 
Here, it is reasonable to use **`self`** as _pseudo-identifier_ path segment. 
For instance, you may define `/employee/self` or `/employee/self/personal-details` 
as resource paths --  and may additionally define endpoints that support 
identifier passing in the resource path, like define `/employee/{id}` 
or `/employee/{id}/personal-details`.
