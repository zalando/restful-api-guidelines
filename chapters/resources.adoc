...
customers' related addresses. We consider `/addresses` and
`/customers/{id}/addresses` as separate resource families because
there's no reliable way to be sure they are the same. We consider
`/customers/{id}/preferences` part of the `/customers` resource type
because it has a one-to-one relation to the customer without an
