...
as a set of highly related resources such as a collection, its members
...
families, one for customers, one for the addresses, and one for the
customers' related addresses. We consider `/customers/{id}/preferences`
part of the `/customers` family because it has a one-to-one relation to
the customer without an additional identifier. We consider `/addresses`
and `/customers/{id}/addresses` as separate resource families because 
there's no reliable way to be sure they are the same.
