...
Following OpenAPI extension for the API Gateway properties *may* be provided in addition:
- `#/info/x-gateway-strip-listen-path` intended target audience of the API (<<219, see rule 219>>)
...
[#250]
== {MAY} strip gateway listen path

Defaults to false if omitted.

If this setting is checked, then the API Gateway will remove the listen path from the inbound URL so that it does not interfere with routing upstream.

[source,yaml]
----
/x-gateway-strip-listen-path:
  type: boolean
  description: |
    If this setting is checked, then the API Gateway will remove the above listen path from the inbound URL so that it does not interfere with routing upstream.
----
...
info:
  x-audience: company-internal
  title: Parcel Helper Service API
x-gateway-strip-listen-path: true
<...>
----
...
[#251]
== {MUST} provide x-gateway-upstream-targets extension

Determines the upstream target based on the environment.
