...
but the API identifier not. Based on the API identifier, we can track the 
API life cycle and manage the history and evolution
of an API as a sequence of API specifications.
For instance, we can provide our API Linter https://github.com/zalando/zally[API Linter Zally]
with capability to check compatibility and correct semantic versioning 
of an API update draft against API running in production.
