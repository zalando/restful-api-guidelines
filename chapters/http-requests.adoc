...
[#253]
== {MAY} use asynchronous request processing

Typically REST APIs are designed as synchronous interfaces where all server-side 
processing and state changes initiated by the call are finished before delivering the 
result as request response. However, in long running request processing situations 
...
via `GET /report-job/{id}` which returns http status code {200} with job status and 
...
Hint: Do _not_ use response code {204} or {404} instead {202} here -- it is missleading
