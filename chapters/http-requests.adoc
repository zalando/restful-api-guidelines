...
separate job resource -- which may happen when your target resource supports
sufficient status monitoring --, you may use `POST /reports` returning a status
code {202} to indicate successful initiation of the asynchronous processing. In
this case the URL from the {Location} header is used to fetch the report via
...
misleading: neither is the processing successfully finished, nor do we want to
