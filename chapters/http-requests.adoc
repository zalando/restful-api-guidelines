...
* The resource identifier {MUST-NOT} be passed as request body data by the client,
...
* If the {POST} creates multiple resources and the creation is atomic, meaning
  either all the resources are created or none of them are, the status code
  {SHOULD} be {201}.
* If the {POST} creates multiple resources and the creation is not atomic,
  meaning some resources can fail to be created, you <<152>>.
