...
* If the response does not contain the created resource or a resource monitor the
  status code should still be {201}, and the URL to {GET} the resource {MUST} be
  provided in the {Location} response header. It is a good practice to always provide
  the {Location} header with the URL to the created resource (see <<180>>).
