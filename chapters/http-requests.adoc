...
**  Assumung that elements don't actually have an `age` but rather a `birthday`
...
APIs that qualify for a specific, complex query language are encouraged to define
it as a structured, potentially nested data structure in JSON.

The reason for favoring data structures over text-based query languages is that
traditionally there are more applications generating queries than there are
that read and interpret them. If a query language is defined in terms of data
structures it's very easy to generate queries, even dynamically. The same is
not true for text-based query languages.

Complex query languages should be specified within the OpenAPI specification
of the API itself. The query is then obviously subject to all
