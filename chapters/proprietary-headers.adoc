...
therefore, the Open API specification is the right place to make this
...
CAUTION: The only exception to this guideline are the conventional
...
[NOTE]
====
This guideline does not standardize proprietary headers for 
...
[NOTE]
====
If a legacy subsystem can only process `Flow-IDs` with a specific
