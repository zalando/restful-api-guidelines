...
Provided by mobile apps only. It is a unique, customer-resettable identifier provided
by mobile device's operating system for advertising purposes. It is not sent if 
the customer disables it in the settings for respective platform. The value 
can either be Apple IDFA (iOS, see 
https://developer.apple.com/documentation/adsupport/asidentifiermanager[Apple developer, 
AdSupport]) or the Goggle AddID (Android, see 
https://support.google.com/admanager/answer/6274238[Goggle Ad Manager]). 
