...
Headers are tyically used to implement protocol processing aspects, such as flow control,
...
in situations where domain-specific generic context information 
a) needs to be passed end to end along the service call chain (even if 
not all called services use it as input for steering service behavior 
e.g. `x-sales-channel` header) and/or b) is provided by specific gateway components, 
for instance, our Fashion Shop API or Merchant API gateway. 
...
(Due to backward compatibility, we did not use e.g. `Zalando-` prefix and 
not followed the Internet Engineering Task Force’s recommendation in {RFC-6648}[RFC 6648] 
to deprecate usage of  `X-` headers). 
...
our specific gateway components (use case b) above). 
