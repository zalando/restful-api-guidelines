...
public class AvoidTrailingSlashesRule implements Rule {
    ...
    private static final String DESCRIPTION = "Rule avoid trailing slashes is not followed for path ";
        ...
        if (swagger != null && swagger.getPaths() != null) {
            swagger.getPaths().keySet().forEach(path -> {
                if (path != null && path.trim().endsWith("/")) {
                    violations.add(new Violation(TITLE, DESCRIPTION + path, MUST, RULE_LINK));
                }
            });
        }
        return violations;
