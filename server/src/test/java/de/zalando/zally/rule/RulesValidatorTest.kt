...
class RulesValidatorTest {
    ...
    fun shouldReturnEmptyViolationsListWithoutRules() {
        val validator = SwaggerRulesValidator(emptyList(), RulesPolicy(emptyArray()))
    ...
    fun shouldReturnOneViolation() {
        ...
        val validator = SwaggerRulesValidator(getRules(violations), RulesPolicy(emptyArray()))
    ...
    fun shouldIgnoreSpecifiedRules() {
        val violations = listOf(DUMMY_VIOLATION_1, DUMMY_VIOLATION_2, DUMMY_VIOLATION_3)
        val validator = SwaggerRulesValidator(getRules(violations), RulesPolicy(arrayOf("M999")))
