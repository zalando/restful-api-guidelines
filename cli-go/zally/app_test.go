...
func TestApp(t *testing.T) {
	t.Run("rules_command_in_the_list", func(t *testing.T) {
		app := CreateApp()
		tests.AssertEquals(t, "rules", app.Commands[0].Name)
	})

	t.Run("has_linter_url_flag", func(t *testing.T) {
		app := CreateApp()
		tests.AssertEquals(t, "linter-service, l", app.Flags[0].GetName())
	})
	...
	t.Run("linter_service_url_can_be_set_from_env", func(t *testing.T) {
		app := CreateApp()
		app.Before = func(c *cli.Context) error {
			tests.AssertEquals(t, "https://localhost:9090", c.String("linter-service"))
			return nil
		}
	...
	t.Run("linter_service_url_can_be_set_from_cli", func(t *testing.T) {
		app := CreateApp()
		app.Before = func(c *cli.Context) error {
			tests.AssertEquals(t, "https://localhost:9091", c.String("linter-service"))
			return nil
		}
	...
	t.Run("has_token_flag", func(t *testing.T) {
		app := CreateApp()
		tests.AssertEquals(t, "token, t", app.Flags[1].GetName())
	})

	t.Run("token_can_be_set_from_env", func(t *testing.T) {
		app := CreateApp()
		app.Before = func(c *cli.Context) error {
			tests.AssertEquals(t, "some-oauth2-token", c.String("token"))
			return nil
		}
